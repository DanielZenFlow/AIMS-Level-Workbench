...
class GridStore {
	...
	undoStack = $state<GridSnapshot[]>([]);
	redoStack = $state<GridSnapshot[]>([]);
	#editGroupSnapshot: GridSnapshot | null = null;
	...
	snapshotsEqual(a: GridSnapshot, b: GridSnapshot): boolean {
		return JSON.stringify(a) === JSON.stringify(b);
	}

	pushUndoSnapshot(snapshot: GridSnapshot) {
	...
	recordEdit() {
		if (this.#editGroupSnapshot) return;
		this.pushUndoSnapshot(this.snapshot());
	}

	beginEditGroup() {
		if (!this.#editGroupSnapshot) this.#editGroupSnapshot = this.snapshot();
	}

	commitEditGroup() {
		if (!this.#editGroupSnapshot) return;
		const snapshot = this.#editGroupSnapshot;
		this.#editGroupSnapshot = null;
		if (!this.snapshotsEqual(snapshot, this.snapshot())) {
	...
	setEntityDetails(row: number, col: number, color: Color, id: string) {
		...
		if (cell.entity.color === color && cell.entity.id === id) {
			return true;
		}
	...
	removeGoal(row: number, col: number): boolean {
		...
		if (!cell || (!isAgentGoalCell(cell) && !isBoxGoalCell(cell))) {
			return false;
		}
	...
	clearCell(row: number, col: number): boolean {
		const cell = this.getCell(row, col);
		if (!cell || !canHaveEntity(cell)) return false;
		if (!cell.entity && !isAgentGoalCell(cell) && !isBoxGoalCell(cell)) return false;
	...
	moveEntity(fromRow: number, fromCol: number, toRow: number, toCol: number): boolean {
		...
		toCell.entity = entity;
		return true;
	}

	moveGoal(fromRow: number, fromCol: number, toRow: number, toCol: number): boolean {
		...
		if (!isAgentGoalCell(fromCell) && !isBoxGoalCell(fromCell)) return false;
		if (!canHaveEntity(toCell)) return false;
		if (isAgentGoalCell(toCell) || isBoxGoalCell(toCell)) return false;

		const goalType = fromCell.type;
		const goalFor = fromCell.goalFor;
		const fromEntity = fromCell.entity;
		const toEntity = toCell.entity;

		this.recordEdit();
		this.grid[toRow][toCol] = {
			id: toCell.id,
			type: goalType,
			goalFor,
			entity: toEntity
		} satisfies CellAgentGoal | CellBoxGoal;
...
const parseColors = (colorsSection: string[]) => {
	...
	colorsSection.forEach((line) => {
		...
		if (!color || !entities) return;
		const ids = entities.split(',').map((id) => id.trim());
		ids.forEach((id) => {
			if (id) colors[id] = color as Color;
		});
	});
	return colors;
};
...
export const fromLevel = (levelString: string) => {
	...
	const requiredSections = ['domain', 'levelname', 'colors', 'initial', 'goal', 'end'];
	const missingSections = requiredSections.filter((section) => !(section in sections));
	if (missingSections.length > 0) {
		throw new Error(`Could not find ${missingSections.map((section) => `#${section}`).join(', ')}.`);
	}
	if (!sections['initial']?.length) throw new Error('The #initial section exists, but its grid is empty.');
	if (!sections['goal']?.length) throw new Error('The #goal section exists, but its grid is empty.');
	...
	if (!initial.length || !initial[0]?.length) throw new Error('The #initial grid is empty.');
	if (goal.length !== initial.length) {
		throw new Error('The #goal grid must have the same number of rows as #initial.');
	}

	const initialGrid = parseLevel(initial, goal, colors);
	const width = initialGrid[0]?.length || 0;
	const height = initialGrid.length;
	const name = sections['levelname']?.[0]?.trim() || 'New level';
